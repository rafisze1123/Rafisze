...
const AdminPage = (() => {
    ...
    app.innerHTML = `
      <div style="min-height:100vh;background:#0d1117;display:flex;
        ...
        <div style="background:#161b22;border:1px solid #30363d;border-radius:16px;
          padding:40px;width:360px;box-shadow:0 8px 24px rgba(0,0,0,0.5);">
          <div style="text-align:center;margin-bottom:28px;">
            <div style="font-size:32px;margin-bottom:8px;">🧺</div>
            <h2 style="color:#fff;font-size:20px;margin:0;font-family:'Inter',sans-serif;">RafiCihuy969</h2>
            <p style="color:#8b949e;font-size:13px;margin:4px 0 0;">Admin Panel</p>
          </div>
          <div style="margin-bottom:14px;">
            <label style="color:#8b949e;font-size:12px;display:block;margin-bottom:4px;">USERNAME</label>
            <input id="inp-user" type="text" placeholder="admin"
              style="width:100%;padding:10px 12px;background:#0d1117;
              border:1px solid #30363d;border-radius:8px;color:#fff;font-size:14px;
              outline:none;box-sizing:border-box;" />
          </div>
          <div style="margin-bottom:18px;">
            <label style="color:#8b949e;font-size:12px;display:block;margin-bottom:4px;">PASSWORD</label>
            <input id="inp-pass" type="password" placeholder="••••••••"
              style="width:100%;padding:10px 12px;background:#0d1117;
              border:1px solid #30363d;border-radius:8px;color:#fff;font-size:14px;
              outline:none;box-sizing:border-box;" />
          </div>
          <p id="login-err" style="display:none;color:#f85149;font-size:12px;
            margin-bottom:12px;text-align:center;">
            Username atau password salah!
          </p>
          <button onclick="AdminPage.doLogin()" style="width:100%;padding:11px;background:#1f6feb;
            border:none;border-radius:8px;color:#fff;font-size:14px;
            font-weight:600;cursor:pointer;transition:background 0.2s;">Masuk</button>
        </div>
      </div>`;
    ...
    return `
      <aside class="sidebar" id="admin-sidebar">
        ...
        <div class="sidebar-footer">
          ...
          <a class="nav-item" style="color:#f85149; margin-top:8px; cursor:pointer;" onclick="AdminPage.doLogout()">
    ...
    container.innerHTML = `
      <div class="admin-page-header">
        <div>
          ...
          <p class="admin-page-subtitle">Ringkasan pendapatan laundry</p>
        </div>
      </div>
      <div class="stats-grid stagger-children" id="pendapatan-stats">
        ${Components.skeletonStats(2)}
      </div>
      <div class="card" style="margin-top:var(--space-6);">
        <div class="table-toolbar">
          <h3 style="margin:0;color:var(--text-primary);font-size:16px;">Riwayat Transaksi</h3>
        </div>
        <div id="pendapatan-table">
          ${Components.skeletonTable(6)}
        </div>
      </div>`;

    await loadPendapatan();
  }
  ...
  async function loadPendapatan() {
    ...
    try {
      ...
      data.forEach(t => {
        const hrg = parseFloat(t.total_harga) || 0;
        totalPendapatanAll += hrg;
        if (t.status === 'Selesai' || t.status === 'Diambil') {
          totalPendapatanSelesai += hrg;
        }
      });

      const statsEl = document.getElementById('pendapatan-stats');
      if (statsEl) {
        statsEl.innerHTML = `
          ${Components.statsCard('💵', Components.formatCurrency(totalPendapatanAll), 'Total Omset (Semua Status)', 'stat-all')}
          ${Components.statsCard('💰', Components.formatCurrency(totalPendapatanSelesai), 'Pendapatan Diterima (Selesai/Diambil)', 'stat-done')}
        `;
      }

      const tableEl = document.getElementById('pendapatan-table');
      if (tableEl) {
        if (data.length === 0) {
          tableEl.innerHTML = Components.emptyState('📋', 'Belum Ada Transaksi', 'Data pendapatan akan muncul di sini');
        } else {
          let html = `
            <div class="table-container">
              <table class="data-table">
          ...
          const sortedData = [...data].sort((a, b) => new Date(b.tanggal_masuk) - new Date(a.tanggal_masuk));
      ...
      Components.toast('Gagal memuat pendapatan', 'error');
    }
  }

  // ======================== BIAYA (LOCAL STORAGE) ========================
  
  let cachedBiaya = JSON.parse(localStorage.getItem('laundry_biaya') || '[]');
    ...
    container.innerHTML = `
      <div class="admin-page-header">
        <div>
          ...
          <p class="admin-page-subtitle">Catat pengeluaran operasional laundry</p>
        </div>
        <button class="btn btn-primary" onclick="AdminPage.showAddBiaya()">+ Tambah Biaya</button>
      </div>
      <div class="stats-grid stagger-children" id="biaya-stats">
        ${Components.skeletonStats(1)}
      </div>
      <div class="card" style="margin-top:var(--space-6);">
        <div class="table-toolbar">
          <h3 style="margin:0;color:var(--text-primary);font-size:16px;">Riwayat Pengeluaran</h3>
        </div>
        <div id="biaya-table">
          ${Components.skeletonTable(4)}
        </div>
  ...
  function loadBiaya() {
    cachedBiaya = JSON.parse(localStorage.getItem('laundry_biaya') || '[]');
    
    let totalBiaya = 0;
    cachedBiaya.forEach(b => totalBiaya += Number(b.nominal));

    const statsEl = document.getElementById('biaya-stats');
    if (statsEl) {
      statsEl.innerHTML = `
        ${Components.statsCard('💸', Components.formatCurrency(totalBiaya), 'Total Pengeluaran', 'stat-expense')}
      `;
    }

    const tableEl = document.getElementById('biaya-table');
    if (tableEl) {
      if (cachedBiaya.length === 0) {
        tableEl.innerHTML = Components.emptyState('📉', 'Belum Ada Pengeluaran', 'Catat pengeluaran operasional Anda di sini');
      } else {
        let html = `
          <div class="table-container">
            <table class="data-table">
        ...
        const sortedData = [...cachedBiaya].sort((a, b) => new Date(b.tanggal) - new Date(a.tanggal));

        sortedData.forEach(b => {
          html += `
            <tr>
              <td>${Components.formatDate(b.tanggal)}</td>
              <td style="color:var(--text-primary);font-weight:500">${b.deskripsi}</td>
              <td style="font-weight:600;color:#f85149">- ${Components.formatCurrency(b.nominal)}</td>
              <td>
                <div class="table-actions">
                  <button class="btn btn-ghost btn-icon" title="Hapus" onclick="AdminPage.confirmDeleteBiaya('${b.id}')">🗑️</button>
                </div>
              </td>
            </tr>`;
        });
    ...
    const body = `
      <form>
        <div class="form-group">
          ...
          <input type="date" class="form-input" id="by-tanggal" required value="${today}">
        </div>
        <div class="form-group">
          <label class="form-label">Deskripsi Pengeluaran</label>
          <input type="text" class="form-input" id="by-deskripsi" required placeholder="Contoh: Beli deterjen 5kg">
        </div>
        <div class="form-group">
          <label class="form-label">Nominal (Rp)</label>
          <input type="number" class="form-input" id="by-nominal" required placeholder="50000" min="0">
        </div>
      </form>`;
  ...
  function submitBiaya() {
    const tgl = document.getElementById('by-tanggal').value;
    const desc = document.getElementById('by-deskripsi').value.trim();
    const nom = document.getElementById('by-nominal').value;
    ...
    cachedBiaya.push(newBiaya);
    localStorage.setItem('laundry_biaya', JSON.stringify(cachedBiaya));
  ...
  function confirmDeleteBiaya(id) {
    Components.confirm('Yakin ingin menghapus catatan pengeluaran ini?', () => {
      cachedBiaya = cachedBiaya.filter(b => b.id !== id);
      localStorage.setItem('laundry_biaya', JSON.stringify(cachedBiaya));
      Components.toast('Catatan biaya berhasil dihapus', 'success');
      loadBiaya();
    });
  }
